...
fun convertOAuth2AuthorizationGrantAuthorization(
    ...
    if (AuthorizationGrantType.AUTHORIZATION_CODE == authorization.authorizationGrantType) {
        val authorizationRequest = authorization
...
fun mapOAuth2DeviceCodeGrantAuthorization(
    ...
    deviceCodeGrantAuthorization.getAuthorizedScopes().let {
        builder.attribute(OAuth2ParameterNames.SCOPE, deviceCodeGrantAuthorization.requestedScopes)
    }
...
fun mapUserCode(userCode: UserCode?, builder: OAuth2Authorization.Builder) {
    if (userCode == null) {
        return
    }
    val oauth2UserCode = OAuth2UserCode(
        userCode.tokenValue, userCode.issuedAt,
        userCode.expiresAt
    )
...
fun mapDeviceCode(
    ...
    val oauth2DeviceCode = OAuth2DeviceCode(
        deviceCode.tokenValue, deviceCode.issuedAt,
...
fun mapIdToken(
    ...
    val oidcIdToken = OidcIdToken(
        idToken.tokenValue, idToken.issuedAt,
        idToken.expiresAt, idToken.claims!!.claims
    )
...
//    val request: OAuth2AuthorizationRequest = authorizationCodeGrantAuthorization.authorizationRequest?.scopes?.let { authorizationCodeGrantAuthorization.authorizationRequest }
//        ?: OAuth2AuthorizationRequest.from(authorizationCodeGrantAuthorization.authorizationRequest).scopes(HashSet()).build()
...
fun mapRefreshToken(
    ...
    val oauth2RefreshToken = OAuth2RefreshToken(
        ...
        refreshToken.issuedAt, refreshToken.expiresAt
    )
...
fun mapAuthorizationCode(
    ...
    val oauth2AuthorizationCode = OAuth2AuthorizationCode(
        ...
        authorizationCode.issuedAt, authorizationCode.expiresAt
    )
...
fun mapAccessToken(
    ...
    val oauth2AccessToken = OAuth2AccessToken(
        ...
        accessToken.tokenValue, accessToken.issuedAt, accessToken.expiresAt,
        accessToken.scopes
    )
...
fun convertOAuth2AuthorizationCodeGrantAuthorization(
    authorization: OAuth2Authorization
    ...
    return OAuth2AuthorizationCodeGrantAuthorization(
        ...
        authorization.authorizedScopes, accessToken, refreshToken,
...
fun convertOAuth2TokenExchangeGrantAuthorization(
    authorization: OAuth2Authorization
): OAuth2TokenExchangeGrantAuthorization {
    val accessToken: AccessToken? = extractAccessToken(authorization)

    return OAuth2TokenExchangeGrantAuthorization(
        authorization.id, authorization.registeredClientId,
        authorization.principalName, authorization.authorizedScopes, accessToken
    )
}


fun convertOAuth2DeviceCodeGrantAuthorization(
    authorization: OAuth2Authorization
    ...
    return OAuth2DeviceCodeGrantAuthorization(
        authorization.id, authorization.registeredClientId,
        authorization.principalName, authorization.authorizedScopes, accessToken, refreshToken,
        authorization.getAttribute<Principal?>(Principal::class.java.getName()), deviceCode, userCode,
        authorization.getAttribute<MutableSet<String?>?>(OAuth2ParameterNames.SCOPE),
        authorization.getAttribute<String?>(OAuth2ParameterNames.STATE)
    )
}

fun convertOAuth2ClientCredentialsGrantAuthorization(
    authorization: OAuth2Authorization
): OAuth2ClientCredentialsGrantAuthorization {
    val accessToken = extractAccessToken(authorization)

    return OAuth2ClientCredentialsGrantAuthorization(
        authorization.id,
        authorization.registeredClientId, authorization.principalName,
        authorization.authorizedScopes, accessToken)
}

fun convertOidcAuthorizationCodeGrantAuthorization(
    authorization: OAuth2Authorization
): OidcAuthorizationCodeGrantAuthorization {
    val authorizationCode: AuthorizationCode? = extractAuthorizationCode(
        authorization
    )
    val accessToken: AccessToken? = extractAccessToken(authorization)
    val refreshToken: RefreshToken? = extractRefreshToken(authorization)
...
fun extractDeviceCode(authorization: OAuth2Authorization): DeviceCode? {
    ...
    if (authorization.getToken(OAuth2DeviceCode::class.java) != null) {
        val oauth2DeviceCode = authorization
            .getToken(OAuth2DeviceCode::class.java)
        deviceCode = DeviceCode(
            oauth2DeviceCode!!.getToken()!!.tokenValue,
            oauth2DeviceCode.getToken()!!.issuedAt, oauth2DeviceCode.getToken()!!.expiresAt,
...
fun extractUserCode(authorization: OAuth2Authorization): UserCode? {
    ...
    if (authorization.getToken(OAuth2UserCode::class.java) != null) {
        ...
        userCode = UserCode(
            ...
            oauth2UserCode.getToken()!!.issuedAt, oauth2UserCode.getToken()!!.expiresAt,
...
fun extractRefreshToken(authorization: OAuth2Authorization): RefreshToken? {
    ...
    if (authorization.refreshToken != null) {
        ...
        refreshToken = RefreshToken(
            oauth2RefreshToken!!.getToken()!!.tokenValue, oauth2RefreshToken.getToken()!!.issuedAt,
            oauth2RefreshToken.getToken()!!.expiresAt, oauth2RefreshToken.isInvalidated
        )
    }
    return refreshToken
}
...
fun extractIdToken(authorization: OAuth2Authorization): OidcAuthorizationCodeGrantAuthorization.IdToken? {
    ...
    if (authorization.getToken(OidcIdToken::class.java) != null) {
        ...
        idToken = OidcAuthorizationCodeGrantAuthorization.IdToken(
            ...
            oidcIdToken.getToken()!!.issuedAt, oidcIdToken.getToken()!!.expiresAt,
...
fun extractAccessToken(authorization: OAuth2Authorization): AccessToken? {
    ...
    if (authorization.accessToken != null) {
        ...
        accessToken = AccessToken(
            oauth2AccessToken.getToken()!!.tokenValue, oauth2AccessToken.getToken()!!.issuedAt!!,
            oauth2AccessToken.getToken()!!.expiresAt!!, oauth2AccessToken.isInvalidated,
            oauth2AccessToken.getToken()!!.tokenType, oauth2AccessToken.getToken()!!.scopes, tokenFormat,
            ClaimsHolder(oauth2AccessToken.claims)
        )
    }
    return accessToken
}
...
fun extractAuthorizationCode(
    ...
    if (authorization.getToken(OAuth2AuthorizationCode::class.java) != null) {
        val oauth2AuthorizationCode = authorization
            .getToken(OAuth2AuthorizationCode::class.java)
        authorizationCode = AuthorizationCode(
            oauth2AuthorizationCode!!.getToken()!!.tokenValue,
            oauth2AuthorizationCode.getToken()!!.issuedAt, oauth2AuthorizationCode.getToken()!!.expiresAt,
