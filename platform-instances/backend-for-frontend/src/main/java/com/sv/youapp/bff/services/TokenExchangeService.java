...
public interface TokenExchangeService {
  ...
  interface RequestSpec<T extends RequestSpec<T>> {
    ...
    /**
     * Sets the redirect URI for the authorization request. The redirect URI is the endpoint to
     * which the authorization server will redirect the user agent after the authorization code,
     * token, or ID token has been obtained.
     *
    ...
    /**
     * Sets the scopes to be included in the authorization request. Scopes define the level of
     * access that the client application is requesting from the resource owner.
     *
    ...
    /**
     * Sets the response type for the redirect request. The response type determines the
     * authorization response flow to be used, such as returning an authorization code, token, or ID
     * token.
     *
     ...
     * authorization response is returned to the client, such as through query parameters, form
     * posts, or fragments.
     *
     ...
     * (Proof Key for Code Exchange) protocol, which enhances the security of OAuth 2.0
     * authorization code flows by adding an additional verification layer during token exchange.
     * This method typically sets or enables the code challenge value, which is derived from the
     * code verifier.
     *
     ...
     * component in the PKCE (Proof Key for Code Exchange) protocol, which enhances security in
     * OAuth 2.0 authorization flows by preventing code interception attacks. It is typically a
     * hashed and encoded value derived from the code verifier.
     *
